...
class RichPolynomial:
    ...
    def _add_splitting_field(self):
        if not hasattr(self, "_RSplF"):
            if self.is_irreducible():
                ...
                max_deg = LCM_list([fac[_sage_const_0 ].deg for fac in self.get_factors_asRP()])
    ...
    # TBD: Function was called get_factors() before!
    ...
    def _add_factors(self):
        self._factors = [(RichPolynomial( g[_sage_const_0 ], self._RFF), g[_sage_const_1 ]) for g in self.get_factorization()]
        return
    ...
    def _add_all_roots(self):    
        self._all_roots = [(fac[_sage_const_0 ].list, (fac[_sage_const_0 ].get_splitting_field()).q, fac[_sage_const_1 ], (fac[_sage_const_0 ].get_RP_in_splitting_field()).roots()) for fac in self.get_factors_asRP()]
        return     
    ...
    def _add_weight(self):
        ...
        self._weight = sum([_sage_const_1  for a in self.list if not a==_sage_const_0 ])
        return 
    ...
    def is_composition(self, m):
        for i in [j for j in range(len(self.list)) if j%m !=_sage_const_0 ]:
            if self.list[i] !=_sage_const_0 :
                return False
        return True
    ...
    def extract(self, m):
        if self.is_composition(m):
            return RichPolynomial([self.list[i] for i in range(len(self.list)) if i%m == _sage_const_0 ], self._RFF) 
        else:
    ...
    def construction_mbetap(self, p):
        ...
        if self.is_composition(p):
            ...
            m_betap_Xp = (-_sage_const_1 )**(self.deg*(p+_sage_const_1 )) * prod([m_beta(zeta_p**j*(self._RFF).x) for j in range(p)]) 
